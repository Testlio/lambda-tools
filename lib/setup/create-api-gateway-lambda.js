...
module.exports = function(region) {
    ...
    return new Promise(function(resolve, reject) {
        const task = logger.task(`Creating lambda execution role ${chalk.underline(ROLE_NAME)}`);
        return ensureLambdaRole(ROLE_NAME, 'inline-policy').then(function(role) {
            task.finish();
            resolve(role);
        }, function(err) {
            task.finish(err);
        ...
        const task = logger.task(`Creating lambda function ${chalk.underline(FUNCTION_NAME)}`);
        ...
        const task = logger.task(`Creating alias ${chalk.underline(alias)}`);
        return updateOrCreateAlias(lambda.FunctionName, lambda.Version, alias).then(function() {
            task.finish();
        }, function(err) {
            task.finish(err);
            throw err;
