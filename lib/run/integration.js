...
module.exports = function *(integrationDefinition) {
    ...
    const methodRequest = {
        path: _.merge({}, this.params),
        querystring: _.merge({}, this.request.query),
        header: _.merge({}, this.request.header)
    };
    ...
    _.forEach(parameters, function(requestKey, integrationKey) {
        if (integrationKey.startsWith('integration.request.')) {
            integrationKey = integrationKey.slice('integration.request.'.length);
        }

        if (requestKey.startsWith('method.request.')) {
    ...
    if (!template) {
        ...
        template = mappingTemplate(template, integration, context, this.request.body);
