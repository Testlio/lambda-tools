...
function wrapper(path, handler, evt, timeout) {
    ...
    const exitFunction = function(error, result) {
        ...
        if (error) {
            sendError(process.pid, error);
            process.exit(1);
        }

        sendResult(process.pid, result);
        process.exit(0);
    };
    ...
    process.on('beforeExit', function(code) {
        if (!exited) {
            sendError(new Error('Process exited without completing request'));
        }
    });

    try {
        reRequire(path)[handler](evt, context);
    } catch(err) {
        sendError(process.pid, err);
        process.exit(1);
    }
}

function sendError(pid, err) {
    console.error(err.message);
    console.error(err.stack);
    process.send({ result: err.message });
}

function sendResult(pid, result) {
