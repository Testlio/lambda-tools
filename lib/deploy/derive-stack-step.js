...
function loadLambdas(result) {
    ...
    // Alias name is the timestamp of the current deployment
    ...
    context.lambdas.forEach(function(lambda) {
        ...
        // Version
        const versionName = camelName + 'Version';

        stack['Resources'][versionName] = JSON.parse(lambdaVersionTemplate({
            resource: 'lambda-tools-lambda-version-resource-beta',
            lambda: camelName,
...
function loadAPI(result) {
    ...
    const parsedTemplate = template({
        lambda: {
            name: 'lambda-tools-api-gateway-resource-beta'
        },
