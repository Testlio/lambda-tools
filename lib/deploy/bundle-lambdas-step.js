...
function getFullDependencies(packages) {
    ...
    if (resolvedDependencies) {
        ...
        promise = new Promise(function(resolve) {
            const child = cp.spawn('npm', ['ls', '--json']);
...
function bundleLambda(options, cwd) {
    return new Promise(function(resolve, reject) {
        ...
        // Envify
        bundler.transform(envify(_.merge({ _: 'purge' }, options.environment)), {
            global: true
        });
        ...
        if (exclusions.length > 0) {
            ...
            if (requires.length > 0) {
                promise = getFullDependencies(requires).then(function(dependencies) {
                    console.log('Adding excluded packages and any dependencies', requires);
