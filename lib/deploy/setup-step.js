...
function prepareStagingDirectory(context) {
    ...
    const stagingDirectory = path.resolve(tmpdir, directoryName);
    const taskName = `Creating staging directory at ${stagingDirectory}`;

    return context.logger.task(taskName, function(resolve, reject) {
        try {
            fsx.createDirectory(stagingDirectory);
        } catch (error) {
            return reject(error);
        }
