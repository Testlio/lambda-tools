...
function listLambdas(context) {
    return new Promise(function(resolve, reject) {
        // Check if we were given a single lambda function in our program
        if (context.program.lambda) {
            const lambdaPath = context.program.lambda.path;
            const name = context.program.lambda.name;
            const handler = 'handler';
            const moduleName = path.basename(lambdaPath, '.js');

            return resolve([{
                name: name,
                module: moduleName,
                handler: handler,
                path: lambdaPath.resolve(context.directories.cwd, path)
            }]);
        }
