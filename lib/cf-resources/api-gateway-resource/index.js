...
exports.handler = function(event, context) {
    ...
    if (command === 'Delete') {
        ...
        if (stageName && deploymentId) {
            promise = promise.then(function(existing) {
                ...
                return APIG.deleteStageAndAPI(existing.id, stageName, deploymentId).then(function() {
                    return existing;
                });
            });
        } else {
    ...
    promise.then(function(existing) {
        ...
        if (existing) {
            return response.send(event, context, response.SUCCESS, existing, existing.deploymentId ? existing.deploymentId : event.PhysicalResourceId);
        } else {
