...
exports.handler = function(event, context) {
    ...
    const localPath = '/Users/henrinormak/Work/Sandbox/api.json';//'/tmp/api.json';
    ...
    // })
    let promise = Promise.resolve(localPath)
    .then(function(filePath) {
    ...
    if (command === 'Delete') {
        ...
        promise = promise.then(function(existing) {
            ...
            if (existing.id && sameStage && sameVariables && !sameDefinitions) {
                ...
                return APIG.createAPI(existing.name, properties.StageName, localPath);
            }
        });
    }
