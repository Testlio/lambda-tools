...
exports.handler = function(event, context) {
    ...
    if (command === 'Delete') {
        ...
        if (stage) {
            ...
            promise = promise.then(function(existing) {
                ...
                return deleteAPIStage(existing.api.id, stage).then(function() {
                    return existing;
                });
            });
        }
        ...
        promise = promise.then(function(existing) {
            return fetchAPIStages(existing.api.id).then(function(stages) {
                if (stages.length === 0) {
                    return deleteAPI(existing.api.id).then(function() {
                        return existing;
                    });
                }
        ...
        promise = promise.then(function(existing) {
            ...
            if (existing.api && sameStage && sameVariables && !sameDefinitions) {
                return Promise.all([
                    ...
                    if (etags.length > 1) {
                        ...
                        return updateAPI(existing.api, existing.name, properties.StageName, localPath);
                    } else {
                        return existing;
                    }
                });
    ...
    promise.then(function(existing) {
        console.log('Done', existing.api);
        return response.send(event, context, response.SUCCESS, existing.api, existing.api.id);
    }).catch(function(err) {
